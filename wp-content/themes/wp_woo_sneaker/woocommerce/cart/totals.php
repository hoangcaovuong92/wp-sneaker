	...
	<?php if ( ! $woocommerce->shipping->enabled || $available_methods || ! $woocommerce->customer->get_shipping_country() || ! $woocommerce->customer->has_calculated_shipping() ) : ?>
		...
		<table cellspacing="0">
			<tbody>
				...
				<?php if ( $woocommerce->cart->get_discounts_before_tax() ) : ?>
					...
					<tr class="discount">
						<th><?php _e( 'Cart Discount', 'wpdance' ); ?> <a href="<?php echo add_query_arg( 'remove_discounts', '1', esc_url( wc_get_cart_url() ) ) ?>"><?php _e( '[Remove]', 'wpdance' ); ?></a></th>
				...
				<?php if ( $woocommerce->cart->get_discounts_after_tax() ) : ?>
					...
					<tr class="discount">
						<th><?php _e( 'Order Discount', 'wpdance' ); ?> <a href="<?php echo add_query_arg( 'remove_discounts', '2', esc_url( wc_get_cart_url() ) ) ?>"><?php _e( '[Remove]', 'wpdance' ); ?></a></th>
						...
						<?php
							...
							if (  $woocommerce->cart->tax_display_cart == 'incl' ) {
								...
								if ( ! empty( $tax_string_array ) ) {
									echo '<small class="includes_tax">' . sprintf( __( '(Includes %s)', 'wpdance' ), implode( ', ', $tax_string_array ) ) . '</small>';
								}
							}
						?>
					</td>
		...
		<?php if ( $woocommerce->cart->get_cart_tax() ) : ?>
			...
			<p><small><?php
				...
				$estimated_text = ( $woocommerce->customer->is_customer_outside_base() && ! $woocommerce->customer->has_calculated_shipping() ) ? sprintf( ' ' . __( ' (taxes estimated for %s)', 'wpdance' ), $woocommerce->countries->estimated_for_prefix() . __( $woocommerce->countries->countries[ $woocommerce->countries->get_base_country() ], 'wpdance' ) ) : '';
