	...
	function wp_comment_form( $args = array(), $post_id = null ) {
		...
		$fields =  array(
			'author' => '<span class="label">Your name</span><p class="comment-form-author">' . '<input id="author" class="input-text" name="author" type="text" value="' .$comment_author. '" defaultvalue="'.$defaut['comment_author'].'" size="30"' . $aria_req . ' />' .
						'</p>',
			'email'  => '<span class="label">Your email</span><p class="comment-form-email"><input id="email" class="input-text" name="email" type="text" value="' . $comment_author_email . '" size="30"' . $aria_req . ' defaultvalue="'.$defaut['comment_author_email'].'"/>'.
						'</p>',
		...
		?>
			<?php if ( comments_open() ) : ?>
				...
				<div id="respond">
					<div class="wd_respond_title"><h3 class="heading-title"><?php comment_form_title( $args['title_reply'], $args['title_reply_to'] ); ?> <small><?php cancel_comment_reply_link( $args['cancel_reply_link'] ); ?></small></h3></div>
