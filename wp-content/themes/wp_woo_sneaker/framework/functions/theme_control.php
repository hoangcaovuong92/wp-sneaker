...
function update_product_archive_template(){
		
	global $smof_data;
	if( isset($smof_data) && isset($smof_data['wd_prod_cat_disc_grid']) && isset($smof_data['wd_prod_cat_disc_list']) ){
		
		if( (int)$smof_data['wd_prod_cat_disc_grid'] == 0 && (int)$smof_data['wd_prod_cat_disc_list'] == 0 ){
			remove_action( 'woocommerce_after_shop_loop_item', 'add_short_content', 5);
		}
		if( ( (int)$smof_data['wd_prod_cat_disc_grid'] == 1 || (int)$smof_data['wd_prod_cat_disc_list'] == 1 ) && ( (int)$smof_data['wd_prod_cat_disc_grid'] + (int)$smof_data['wd_prod_cat_disc_list'] == 1 ) ){
...
function revert_update_product_archive_template(){
	global $smof_data;
	if( isset($smof_data) && isset($smof_data['wd_prod_cat_disc_grid']) && isset($smof_data['wd_prod_cat_disc_list']) ){
		if( (int)$smof_data['wd_prod_cat_disc_grid'] == 0 && (int)$smof_data['wd_prod_cat_disc_list'] == 0 ){
		...
		if( ( (int)$smof_data['wd_prod_cat_disc_grid'] == 1 || (int)$smof_data['wd_prod_cat_disc_list'] == 1 ) && ( (int)$smof_data['wd_prod_cat_disc_grid'] + (int)$smof_data['wd_prod_cat_disc_list'] == 1 ) ){
			//remove_action( 'wp_enqueue_scripts', 'custom_list_grid_disc' );
		}
	}
...
function custom_list_grid_disc() {
	global $smof_data;
	$custom_css = "";
	if( (int)$smof_data['wd_prod_cat_disc_list'] == 0 ){
		$custom_css = "ul.list .short-description{display: none!important;}";
	}	
	if( (int)$smof_data['wd_prod_cat_disc_grid'] == 0 ){
		$custom_css .= "ul.grid .short-description{display: none!important;}";
	}   
...
function my_page_template_redirect(){
	...
	if($wp_query->is_page()){
		global $page_datas,$wd_custom_style_config,$smof_data;
	...
	if( is_tax( 'product_cat' ) || is_tax( 'product_tag' ) || is_post_type_archive( "product" ) ){
		...
		//add_action('woocommerce_before_main_content','update_product_archive_template', 10);
		//add_action('woocommerce_after_main_content','revert_update_product_archive_template', 10);
...
//					add_action( 'get_header',  'wd_init_sidebar_replacement' );
		...
		if( !$smof_data['wd_prod_title'] )
			remove_action( 'woocommerce_before_single_product_summary', 'woocommerce_template_single_title', 1 );
		
		if( !$smof_data['wd_prod_email_friend'] )
			remove_action( 'woocommerce_single_product_summary', 'wd_template_single_sharing_second', 17 );
		...
		if( !$smof_data['wd_prod_share'] )
			remove_action( 'woocommerce_single_product_summary', 'wd_template_single_sharing_first', 9 );
	...
	/*Check remove content under menu*/
	if( in_array( "woocommerce/woocommerce.php", apply_filters( 'active_plugins', get_option( 'active_plugins' )  ) ) ){
		if( is_woocommerce() || is_cart() || is_checkout() || is_account_page() ){
			remove_action( 'wd_bofore_main_container', 'wd_print_content_under_menu', 15 );
		}
	}
}
...
function wd_remove_shop_archive_control(){
		...
		/*$show_discription = $smof_data['wd_prod_cat_disc_grid'];
		if( $show_discription == 0 )
