...
class EWAbstractTheme 
{
	...
	protected function init(){
		...
		add_action( 'wp_head', array($this,'register_google_font') );
	...
	protected function initArrFunctions(){
		$this->arrFunctions = array('custom_functions','main','global_var','preview_mod','ads','filter_editor','quicksand','slide','markup_categories','lightbox_control',
		'breadcrumbs','sidebar','twitter_update','feed_burner','excerpt','theme_control','pagination','filter_theme','posted_in_on',
		'video','comment','theme_sidebar','logo_function','wdmenus','woo-cart','woo-product','woo-hook','woo-account','custom_term');
	}
	...
	public function wpdancesetup() {
		...
		register_nav_menus( array(
			'footer' => __( 'Footer Navigation', 'wpdance' ),
		) );
		register_nav_menus( array(
	...
	function load_gg_fonts( $wd_font_name = "" ) {
		if( isset($wd_font_name) && strlen( $wd_font_name ) > 0 ){
			...
			wp_enqueue_style( "sneaker-{$font_name_id}", "{$protocol}://fonts.googleapis.com/css?family={$wd_font_name}" );		
		}
	}
	...
	function register_google_font(){
		...
		if( absint($smof_data['wd_heading_font_googlefont_enable']) == 0 && strcmp($heading_font,'none') != 0 )
			$this->load_gg_fonts(trim($heading_font));
	...
	public function addScripts(){
		...
		wp_enqueue_style( 'sneaker-open', "$protocol://fonts.googleapis.com/css?family=Open+Sans:400,600,700" );
		wp_enqueue_style( 'sneaker-advent', "$protocol://fonts.googleapis.com/css?family=Advent+Pro:400,600,700,500" );
		
		wp_enqueue_style( 'sneaker', get_stylesheet_uri() ); 
		...
		wp_enqueue_style('wd-update-css', 	THEME_CSS.'/wd_update.css');	
	}
}
?>