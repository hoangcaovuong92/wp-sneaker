...
class WD_Quickshop 
{
	...
	public function add_quickshop_button(){
		...
		if( isset($smof_data) && isset( $smof_data['wd_qs_button_imgage'] ) && strlen( trim($smof_data['wd_qs_button_imgage']) ) > 0 ){
			$btn_img = "<img id='em_quickshop_handler_img' src='" . esc_url($smof_data['wd_qs_button_imgage']) . "' title='{$btn_label}'>";
		}	
...
?>
		...
		<a class="visible-desktop" id="em_quickshop_handler" href="#" style="display:none;position:absolute;top:-1000px;left:-1000px;opacity:0;z-index:999;">
			<span id="qs_inner1">
				<span id="qs_inner2" class="<?php echo $qs_class = strlen(trim($btn_img)) > 0 ? "qs_img_btn" : "qs_text_btn" ;?>"> 
					<?php 
...
<?php	
	...
	protected function init_trigger(){
		...
		add_action( 'wd_quickshop_single_product_summary', array($this,'wd_quickshop_product_availability'), 7 );
		add_action( 'wd_quickshop_single_product_summary', array($this,'wd_qs_template_single_sku'), 8 );
		add_action( 'wd_quickshop_single_product_summary', 'woocommerce_template_loop_rating', 6 );
	...
	public function load_product_content_callback(){
		...
		if( isset($smof_data['wd_qs_product_availability']) && $smof_data['wd_qs_product_availability'] == 0 )
			remove_action( 'wd_quickshop_single_product_summary', array($this,'wd_quickshop_product_availability'), 7 );
		if( isset($smof_data['wd_qs_product_sku']) && $smof_data['wd_qs_product_sku'] == 0 )
			remove_action( 'wd_quickshop_single_product_summary', array($this,'wd_qs_template_single_sku'), 8 );
		if( isset($smof_data['wd_qs_product_rating']) && $smof_data['wd_qs_product_rating'] == 0 )
			remove_action( 'wd_quickshop_single_product_summary', 'woocommerce_template_loop_rating', 6 );
