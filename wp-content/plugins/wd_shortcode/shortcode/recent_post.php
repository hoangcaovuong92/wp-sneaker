...
if(!function_exists ('recent_blogs_functions')){
	function recent_blogs_functions($atts,$content = false){
		...
		if( have_posts() ) :
			...
			ob_start();
			echo '<ul id="'. $id_widget .'" class="shortcode-recent-blogs columns-'.$columns.' '.$style.' ">';
			$i = 0;
			while(have_posts()) {
				the_post();
				global $post;
				...
				<?php if( strcmp($style,'small-title') == 0):?>
						...
						<div class="detail<?php if(!$thumbnail) echo ' noimage';?>">
							...
							<p class="date-time <?php if(!$excerpt) echo 'hidden-element'; ?>"><?php the_time(get_option('date_format')); ?></p>
							<p class="excerpt <?php if(!$excerpt) echo 'hidden-element'; ?>"><?php the_excerpt_max_words($excerpt_words); ?></p>
						</div>	
				<?php endif;?>
				
				<?php if( strcmp($style,'big-title') == 0 ):?>
						...
						<div class="detail<?php if(!$thumbnail) echo ' noimage';?>">
							...
							<div class="bottom-detail <?php if(!$meta) echo 'hidden-element'; ?>">
							<span class="author">	
...
if(!function_exists ('recent_blogs_slider_function')){
	function recent_blogs_slider_function($atts,$content = false){
		extract(shortcode_atts(array(
			'title'						=> ''
			,'category'					=> ''
			,'columns'					=> 4
		...
		$show_thumbnail = (int)$show_thumbnail;
		$show_meta = (int)$show_meta;
		$show_view_more_button = (int)$show_view_more_button;
		$show_excerpt = (int)$show_excerpt;
		
		$num_count = count(query_posts($args));	
		if( have_posts() ) :
			$_random_id = 'recent-blogs-shortcode-slider-'.rand(0,1000);
			ob_start();
				?>
				<div id="<?php echo $_random_id; ?>" class="shortcode-recent-blogs-slider loading <?php echo ($columns==1)?'single-item':''; ?>">
					<div class="wd-blog-title"><h3 class="heading-title"><?php echo $title; ?></h3></div>
						...
						<li class="item">
							...
							<div class="image">
								<a class="thumbnail" href="<?php the_permalink(); ?>">
									<div class="bg-img">
									<?php 
										$thumbnai_size = ($columns == 1)?'blog_shortcode_slider':'blog_shortcode';
										the_post_thumbnail($thumbnai_size,array('class' => 'thumbnail-effect-1'));
									?>
									</div>
								</a>
							</div>
							<?php } ?>
							<div class="detail<?php if(!$show_thumbnail) echo ' noimage';?>">
								<?php if( $show_post_title ){ ?>
									<h4 class="heading-title"><a href="<?php echo get_permalink($post->ID); ?>" class="wpt_title"  ><?php echo get_the_title($post->ID); ?></a></h4>
								<?php } ?>
								<?php  if($show_excerpt){ ?>
								...
								<a class="view-more" href="<?php echo get_permalink($post->ID); ?>"><?php _e('View more','wpdance'); ?></a>
								<?php } ?>
							</div>	
						</li>
						<?php } ?>
				...
				<script type="text/javascript">
					jQuery(document).ready(function(){
						...
						var single_item = <?php echo $columns; ?> == 1;
						var owl = $_this.find('ul.recent-blogs').owlCarousel({
...
add_shortcode('recent_blogs_slider','recent_blogs_slider_function');
